    ...
    if event.key == 'w':
        ...
        #checks if in range of ingredient boxes
            ...
            #checks if in range of counter, if so then drop the item
                ...
                or inRange(app, app.chef1, app.chopBoard) or inRange(app, app.chef1, app.stove)):
                pickUp(app)
                placeOnCounter(app)
    elif event.key == 'Tab':
...
def pickUp(app):
    ...
    if otherIngred != None:
        if (isinstance(app.chef1.holding, ingredient.Burger) and 
            ...
            app.chef1.holding = ingredient.Burger({app.chef1.holding, otherIngred})
    print(type(app.chef1.holding))
...
def inRange(app, chef, box):
    if chef.x-chef.r == box.x1 and box.y0 <= chef.y <= box.y1: 
        return True #left side of counter
